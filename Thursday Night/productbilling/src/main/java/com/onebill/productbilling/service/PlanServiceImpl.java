...
public class PlanServiceImpl implements PlanService {
	...
	public PlanRespDto addPlan(PlanDto plan) {
		...
		try {
			...
			throw new BillingException("Failed to add plan of given type (" + plan.getType()
					+ ") and for given product. Please check with Your Inputs");
		}
	}
	...
	public PlanRespDto updatePlan(PlanDto plan) {
		...
		try {
			...
			if(plan.getPlanAmount()==0.0 || plan.getPlanFrequency()==0) {
				throw new BillingException("Failed to Update plan with Amount ("+plan.getPlanAmount()+") and Frequency ("+plan.getPlanFrequency()+")");
			}else {
		...
		try {
			...
			if (plan.getDetail() == 0) {
				...
				throw new BillingException("Failed to update plan detail. Please check with Your inputs. type ("
						+ plan.getServiceType() + ") and detail (" + plan.getDetail() + ") and unit ("
						+ plan.getUnit() + ") and for Given Plan ");
